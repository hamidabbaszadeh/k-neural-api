    ...
    def __call__(self, img, steps, step_size):
        ...
        for n in tf.range(steps):
            with tf.GradientTape() as tape:
                ...
                tape.watch(img)
                loss = tf.cast(
                    calc_sum_channel_mean_from_model(img, self.model), dtype=tf.float32
                )
...
def run_gradient_ascent_octaves(
    img,
    partial_model,
    steps=50,
    step_size=0.01,
    ...
    for n in range(low_range, high_range):
        ...
        img = run_gradient_ascent(
            img=img, partial_model=partial_model, steps=steps, step_size=step_size
        )
    return img
