...
def plant_leaf(
    pinput_shape, num_classes, l2_decay=0.0, dropout_drop_rate=0.2, has_batch_norm=True
):
    ...
    if has_batch_norm:
        last_tensor = keras.layers.BatchNormalization(axis=3, epsilon=1.001e-5)(
            last_tensor
        )
...
def CreatePartialModelCopyingChannels(
    pModel, pOutputLayerName, pChannelStart, pChannelCount
):
    ...
    inputs = pModel.input
    outputs = pModel.get_layer(pOutputLayerName).output
    outputs = cai.layers.CopyChannels(
        channel_start=pChannelStart, channel_count=pChannelCount
    )(outputs)
    return keras.Model(inputs=inputs, outputs=outputs)


def CreatePartialModelWithSoftMax(
    pModel, pOutputLayerName, numClasses, newLayerName="k_probs"
):
    ...
    outputs = keras.layers.Dense(numClasses, activation="softmax", name=newLayerName)(
        outputs
    )
...
def CreatePartialModelFromChannel(pModel, pOutputLayerName, pChannelIdx):
    ...
    """
    return CreatePartialModelCopyingChannels(
        pModel, pOutputLayerName, pChannelStart=pChannelIdx, pChannelCount=1
    )


def PartialModelPredict(
    aInput, pModel, pOutputLayerName, hasGlobalAvg=False, pBatchSize=32
):
    ...
    layeroutput = np.array(
        IntermediateLayerModel.predict(x=aInput, batch_size=pBatchSize)
    )
    IntermediateLayerModel = 0
    return layeroutput


def calculate_heat_map_from_dense_and_avgpool(
    aInput, target_class, pModel, pOutputLayerName, pDenseLayerName
):
    ...
    conv_output = cai.models.PartialModelPredict(
        localImageArray, pModel, pOutputLayerName
    )[0]
