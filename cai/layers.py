...
class CopyChannels(tensorflow.keras.layers.Layer):
    ...
    def __init__(self, channel_start=0, channel_count=1, **kwargs):
    ...
    def call(self, x):
        return x[
            :, :, :, self.channel_start : (self.channel_start + self.channel_count)
        ]
...
class InterleaveChannels(tensorflow.keras.layers.Layer):
    ...
    def __init__(self, step_size=2, **kwargs):
        if step_size < 2:
            self.step_size = 1
        else:
    ...
    def call(self, x):
        return tensorflow.keras.layers.Concatenate(axis=3)(
            [
                x[:, :, :, shift_pos :: self.step_size]
                for shift_pos in range(self.step_size)
            ]
        )
...
def GlobalAverageMaxPooling2D(previous_layer, name=None):
    ...
    return tensorflow.keras.layers.Concatenate(axis=1)(
        [
            tensorflow.keras.layers.GlobalAveragePooling2D(name=name + "_avg")(
                previous_layer
            ),
            tensorflow.keras.layers.GlobalMaxPooling2D(name=name + "_max")(
                previous_layer
            ),
        ]
    )
...
def FitChannelCountTo(
    last_tensor, next_channel_count, has_interleaving=False, channel_axis=3
):
    """
    ...
    """
    prev_layer_channel_count = tensorflow.keras.backend.int_shape(last_tensor)[
        channel_axis
    ]
        ...
        if copy_cnt == 0:
            ...
            if has_interleaving:
                output_copies.append(
                    InterleaveChannels(
                        step_size=((copy_cnt + 1) % prev_layer_channel_count)
                    )(last_tensor)
                )
            else:
                output_copies.append(last_tensor)
    if extra_channels > 0:
        if has_interleaving:
            extra_tensor = InterleaveChannels(
                step_size=((full_copies + 1) % prev_layer_channel_count)
            )(last_tensor)
        else:
...
def BinaryConvLayers(
    ...
    for conv_cnt in range(conv_count):
        ...
        if has_interleaving:
            last_tensor_interleaved = InterleaveChannels(
                step_size=2, name=name + "_i_" + str(conv_cnt)
            )(last_tensor)
        else:
            last_tensor_interleaved = last_tensor
        x1 = tensorflow.keras.layers.Conv2D(
            prev_layer_channel_count // 2,
            shape,
            padding="same",
            activation=None,
            name=name + "_a_" + str(conv_cnt),
            groups=prev_layer_channel_count // 2,
        )(last_tensor)
        x2 = tensorflow.keras.layers.Conv2D(
            prev_layer_channel_count // 2,
            shape,
            padding="same",
            activation=None,
            name=name + "_b_" + str(conv_cnt),
            groups=prev_layer_channel_count // 2,
        )(last_tensor_interleaved)
        last_tensor = tensorflow.keras.layers.Concatenate(
            axis=channel_axis, name=name + "_conc_" + str(conv_cnt)
        )([x1, x2])
        if has_batch_norm:
            last_tensor = tensorflow.keras.layers.BatchNormalization(
                axis=channel_axis, name=name + "_batch_" + str(conv_cnt)
            )(last_tensor)
        if activation is not None:
            last_tensor = tensorflow.keras.layers.Activation(
                activation=activation, name=name + "_act_" + str(conv_cnt)
            )(last_tensor)
        from_highway = tensorflow.keras.layers.DepthwiseConv2D(
            1,  # kernel_size
            strides=1,
            padding="valid",
            use_bias=False,
            name=name + "_depth_" + str(conv_cnt),
        )(input_tensor)
        last_tensor = tensorflow.keras.layers.add(
            [from_highway, last_tensor], name=name + "_add" + str(conv_cnt)
        )
        if has_batch_norm:
            last_tensor = tensorflow.keras.layers.BatchNormalization(axis=channel_axis)(
                last_tensor
            )
    return last_tensor


def BinaryPointwiseConvLayers(
    last_tensor,
    name,
    conv_count=1,
    has_batch_norm=True,
...
def BinaryCompressionLayer(
    last_tensor, name, has_batch_norm=True, activation="relu", channel_axis=3
):
    last_tensor = EnforceEvenChannelCount(last_tensor)
    prev_layer_channel_count = tensorflow.keras.backend.int_shape(last_tensor)[
        channel_axis
    ]
    last_tensor = tensorflow.keras.layers.Conv2D(
        prev_layer_channel_count // 2,
        (1, 1),
        padding="same",
        activation=None,
    ...
    if has_batch_norm:
        last_tensor = tensorflow.keras.layers.BatchNormalization(
            axis=channel_axis, name=name + "_batch"
        )(last_tensor)
    if activation is not None:
        last_tensor = tensorflow.keras.layers.Activation(
            activation=activation, name=name + "_act"
        )(last_tensor)
    return last_tensor


def BinaryCompression(
    last_tensor,
    name,
    ...
    while prev_layer_channel_count >= target_channel_count * 2:
        last_tensor = BinaryCompressionLayer(
            last_tensor,
            name=name + "_" + str(cnt),
            has_batch_norm=has_batch_norm,
            activation=activation,
    ...
    if prev_layer_channel_count > target_channel_count:
        last_tensor = FitChannelCountTo(
            last_tensor,
            next_channel_count=target_channel_count * 2,
            channel_axis=channel_axis,
        )
        last_tensor = BinaryCompressionLayer(
            last_tensor,
...
def conv2d_bn(
    ...
    if has_batch_norm:
        x = tensorflow.keras.layers.BatchNormalization(
            axis=bn_axis, scale=has_batch_scale, name=bn_name
        )(x)
    if activation is not None:
        x = tensorflow.keras.layers.Activation(activation=activation, name=name)(x)
    return x
...
def HardSwish(x):
    ...
    """
    return tensorflow.keras.layers.Multiply()(
        [tensorflow.keras.layers.Activation(HardSigmoid)(x), x]
    )
...
def kConv2DType1(
    ...
    group_count = cai.util.get_max_acceptable_common_divisor(
        prev_layer_channel_count,
    ...
    if group_count > 1:
        ...
        if output_group_size > 1:
            output_tensor = InterleaveChannels(
                output_group_size, name=name + "_group_interleaved"
            )(output_tensor)
    else:
...
def kConv2DType2(
    ...
    if group_count > 1:
        ...
        if prev_layer_channel_count >= output_channel_count:
            ...
            output_tensor = tensorflow.keras.layers.add(
                [output_tensor, compression_tensor], name=name + "_inter_group_add"
            )
    else:
        # print ('Dismissed groups:', group_count, 'Input channels:', prev_layer_channel_count, 'Output Channels:', output_channel_count, 'Input channels per group:', input_group_size, 'Output channels per group:', output_group_size)
        output_tensor = conv2d_bn(
...
def kConv2DType6(
    ...
    if group_count > 1:
        ...
        if prev_layer_channel_count >= output_channel_count:
            ...
            output_tensor = tensorflow.keras.layers.Concatenate(
                axis=3, name=name + "_inter_group_concat"
...
def kConv2DType7(
    ...
    if filters >= prev_layer_channel_count:
        if filters > prev_layer_channel_count:
            last_tensor = FitChannelCountTo(
                last_tensor, next_channel_count=filters, channel_axis=channel_axis
            )
        if bin_conv_count > 0:
            last_tensor = BinaryPointwiseConvLayers(
                last_tensor,
                name,
    ...
    if filters < prev_layer_channel_count:
        ...
        last_tensor = BinaryCompression(
            last_tensor,
            name=name + "_bicompress",
            activation=activation,
            has_batch_norm=has_batch_norm,
            target_channel_count=filters,
        )
    return last_tensor
...
def kConv2DType8(
    ...
    if group_count > 1:
        ...
        compression_tensor = output_tensor
        second_conv_max_acceptable_divisor = (
            output_channel_count // min_channels_per_group
        )
        second_conv_group_count = cai.util.get_max_acceptable_common_divisor(
            output_channel_count,
            output_channel_count,
        ...
        if second_conv_group_count > 1:
            if (prev_layer_channel_count >= output_channel_count) or (
                always_intergroup
            ):
                # print('Has grouped intergroup')
                if activation is None:
                    output_tensor = tensorflow.keras.layers.Activation(HardSwish)(
                        output_tensor
                    )
                output_tensor = InterleaveChannels(
                    output_group_size, name=name + "_group_interleaved"
                ...
                output_tensor = conv2d_bn(
                    ...
                    has_batch_norm=has_batch_norm,
                    has_batch_scale=has_batch_scale,
                    groups=second_conv_group_count,
                    use_bias=use_bias,
                )
        else:
            # print('Has intergroup')
...
def kConv2DType9(
    ...
    output_channel_count = filters
    first_conv_min_group_channel_count = max(
        min_channels_per_group, int(math.sqrt(prev_layer_channel_count))
    )
    max_acceptable_divisor = (
        prev_layer_channel_count // first_conv_min_group_channel_count
    )
    group_count = cai.util.get_max_acceptable_common_divisor(
        prev_layer_channel_count,
        output_channel_count,
    ...
    if group_count > 1:
        ...
        compression_tensor = output_tensor
        second_conv_min_group_channel_count = max(
            min_channels_per_group, int(math.sqrt(output_channel_count))
        )
        second_conv_max_acceptable_divisor = (
            output_channel_count // second_conv_min_group_channel_count
        )
        second_conv_group_count = cai.util.get_max_acceptable_common_divisor(
            output_channel_count,
            output_channel_count,
...
def kGroupConv2D(
    ...
    if groups > 1:
        ...
        if groups * channels_per_group > prev_layer_channel_count:
            last_tensor = FitChannelCountTo(
                last_tensor,
                next_channel_count=groups * channels_per_group,
                has_interleaving=False,
                channel_axis=channel_axis,
            )
        ...
        if groups * channels_per_group < prev_layer_channel_count:
            ...
            if local_channels_per_group * groups > prev_layer_channel_count:
                last_tensor = FitChannelCountTo(
                    last_tensor,
                    next_channel_count=groups * local_channels_per_group,
                    has_interleaving=False,
                    channel_axis=channel_axis,
                )
        prev_layer_channel_count = tensorflow.keras.backend.int_shape(last_tensor)[
        ...
        if extra_filters == 0:
            last_tensor = conv2d_bn(
                last_tensor,
                filters - extra_filters,
                kernel_size,
                kernel_size,
                name=name + "_m" + str(groups),
                activation=activation,
                has_batch_norm=has_batch_norm,
                has_batch_scale=has_batch_scale,
                use_bias=use_bias,
            ...
            path2 = conv2d_bn(
                ...
                padding=padding,
                strides=(stride_size, stride_size),
                groups=extra_filters,
            )
            # concats both paths.
            last_tensor = tensorflow.keras.layers.Concatenate(
                axis=3, name=name + "_dc"
            )([path1, path2])  # deep concat
    else:
        # deep unmodified.
        last_tensor = conv2d_bn(
            last_tensor,
...
def kConv2DType10(
    ...
    if (prev_layer_channel_count > 2 * min_channels_per_group) or (
        ...
        output_tensor, group_count = kGroupConv2D(
            output_tensor,
            filters=filters,
            channel_axis=channel_axis,
        ...
        if (
            (group_count > 1)
            and (prev_layer_channel_count >= filters)
            and not (never_intergroup)
        ):
            compression_tensor = output_tensor
            ...
            if interleave_step > 1:
                output_tensor = InterleaveChannels(
                    interleave_step, name=name + "_i" + str(interleave_step)
                )(output_tensor)
            output_tensor, group_count = kGroupConv2D(
                output_tensor,
                filters=filters,
            ...
            output_tensor = tensorflow.keras.layers.add(
                [output_tensor, compression_tensor], name=name + "_iga"
            )
    else:
        # unmofied
        output_tensor = conv2d_bn(
