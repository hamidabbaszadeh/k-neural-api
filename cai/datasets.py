    ...
    return np.array(
        cv2.warpAffine(
            ...
            cv2.getRotationMatrix2D(
                (image.shape[0] / 2 - 0.5, image.shape[1] / 2 - 0.5), angle, 1.0
            ),
...
def occlusion_a(aImages, pixels=10, verbose=False):
    ...
    for img in range(imgLen):
        ...
        occlusion(
            image=aImages[img], startx=startx, starty=starty, lenx=pixels, leny=pixels
        )
        if img % 1000 == 0:
            gc.collect()
...
def motion_blur_a(aImages, pixels=10, dtype="float32", verbose=False):
    ...
    imgLen = len(aImages)
    for img in range(imgLen):
        aImages[img] = motion_blur(
            image=aImages[img], size=pixels, angle=random.randint(0, 180), dtype=dtype
        )
        if img % 1000 == 0:
            gc.collect()
            if verbose and (img > 0):
    ...
    for img in range(imgLen):
        ...
        for x in range(xLen):
            for y in range(yLen):
                ...
                g = aRGB[img][x][y][1]
                b = aRGB[img][x][y][2]
                lightness, a, bb = rgb2lab(r, g, b)
                aRGB[img][x][y][0] = lightness
                aRGB[img][x][y][1] = a
                aRGB[img][x][y][2] = bb
...
def cv2_resize_a(
    aImages, target_size=(64, 64), interpolation=cv2.INTER_NEAREST, verbose=False
):
    ...
    for img in range(imgLen):
        outImg.append(
            cv2.resize(aImages[img], dsize=target_size, interpolation=interpolation)
        )
        if img % 1000 == 0:
            gc.collect()
            if verbose:
...
def salt_pepper(
    aImages, salt_pepper_num, salt_value=2.0, pepper_value=-2.0, verbose=True
):
...
class img_folder_dataset:
    ...
    def load_data(self):
        ...
        for folder in folder_list:
            if len(self.folder_starts_with) == 0 or folder.startswith(
                self.folder_starts_with
            ):
                cnt_per_class = 0
                ...
                for img_file in img_list:
                    ...
                    if absolute_file_name.lower().endswith(".jpg"):
                        aimage = img_to_array(
                            cv2.resize(
                                cv2.imread(absolute_file_name),
                                tuple((self.sizex, self.sizey)),
                            ),
                            dtype="int16",
                        )
                        # print(aimage.shape)
                        ...
                        if (
                            ...
                            and cnt_per_class >= self.max_samples_per_class
                        ):
                            break
                label_id = label_id + 1
        # print(image_list)
        ...
        x_train, x_test, y_train, y_test = train_test_split(
            image_list, label_list, test_size=self.test_size, random_state=17
        )
...
def Create2DGaussian(SizeX, SizeY, DistScale=1, ResultScale=1, ResultBias=0):
    ...
    for i in range(SizeX):
        for j in range(SizeY):
            distance = (
                math.sqrt((i - (SizeX / 2)) ** 2 + (j - (SizeY / 2)) ** 2) * DistScale
            )
            s[i][j] = rv.pdf(distance) * ResultScale + ResultBias
    return s


def Create2DGaussianWithChannels(
    SizeX, SizeY, DistScale=1, ResultScale=1, ResultBias=0, Channels=1
):
...
def load_dataset_with_validation(
    ...
    base_model_name="",
    validation_size=0.1,
    validation_flip_horizontal=False,
...
def load_cifar10_dataset(lab=False, verbose=False, bipolar=True):
    ...
    """
    return load_dataset(
        cifar10, lab=lab, verbose=verbose, bipolar=bipolar, base_model_name="cifar10"
    )


def save_dataset_in_format(
    aImages,
    aClasses,
    dest_folder_name="img",
    format=".png",
    ...
    for img_cnt in range(imgLen):
        ...
        if with_horizontal_flip:
            cv2.imwrite(
                class_folder + "/h_img_" + str(img_cnt) + format, np.flip(img, 1)
            )
        if with_vertical_flip:
            cv2.imwrite(
                class_folder + "/v_img_" + str(img_cnt) + format, np.flip(img, 0)
            )
        if with_horizontal_flip and with_vertical_flip:
            cv2.imwrite(
                class_folder + "/hv_img_" + str(img_cnt) + format,
...
def fix_bad_tfkeras_channel_order(aImages):
    ...
    """
    local_x = np.zeros(
        shape=(aImages.shape[0], aImages.shape[1], aImages.shape[2], aImages.shape[3])
    )
    local_x[:, :, :, 0] = aImages[:, :, :, 2]
    local_x[:, :, :, 1] = aImages[:, :, :, 1]
    local_x[:, :, :, 2] = aImages[:, :, :, 0]
...
def save_tfds_in_format(
    ...
    for x in p_tfds.as_numpy_iterator():
        ...
        if with_horizontal_flip:
            cv2.imwrite(class_folder + "/h_img_" + sample_idx + format, np.flip(img, 1))
        if with_vertical_flip:
            cv2.imwrite(class_folder + "/v_img_" + sample_idx + format, np.flip(img, 0))
        if with_horizontal_flip and with_vertical_flip:
            cv2.imwrite(
                class_folder + "/hv_img_" + sample_idx + format,
                np.flip(np.flip(img, 0), 1),
            )
        cnt = cnt + 1


def save_dataset_as_png(
    aImages,
    aClasses,
    dest_folder_name="img",
    with_horizontal_flip=False,
    ...
    save_dataset_in_format(
        aImages,
        aClasses,
        dest_folder_name=dest_folder_name,
        format=".png",
        with_horizontal_flip=with_horizontal_flip,
        with_vertical_flip=with_vertical_flip,
    )


def save_dataset_as_jpg(
    aImages,
    aClasses,
    dest_folder_name="img",
    with_horizontal_flip=False,
...
def save_tfds_as_png(
    p_tfds, dest_folder_name="img", with_horizontal_flip=False, with_vertical_flip=False
):
...
def download_zip_and_extract(
    url_zip_file, local_zip_file, expected_folder_name, Verbose=True
):
    ...
    if not os.path.isfile(local_zip_file):
        if Verbose:
            print("Downloading: ", url_zip_file, " to ", local_zip_file)
        download_file(url_zip_file, local_zip_file)
    if not os.path.isdir(expected_folder_name):
        os.mkdir(expected_folder_name)
        if Verbose:
            print("Decompressing into: ", expected_folder_name)
        unzip_file(local_zip_file, expected_folder_name)


def load_hyperspectral_matlab_image(
    ImgUrl, ClassUrl, ImgProperty, ClassProperty, LocalBaseName, Verbose=True
):
...
def slice_image(Image, PixelClasses, NewImageSize=5):
    ...
    aResultImg = [
        Image[X : X + NewImageSize, Y : Y + NewImageSize, :]
        for X in range(0, MaxX)
        for Y in range(0, MaxY)
    ]

    aResultClasses = [
        int(PixelClasses[X + Center, Y + Center])
        for X in range(0, MaxX)
        for Y in range(0, MaxY)
    ]
...
def slice_images(Images, NewImageSize=3):
    ...
    aResultImg = [
        Images[ImagePos, X : X + NewImageSize, Y : Y + NewImageSize, :]
...
def train_model_on_dataset(
    ...
    """
    x_train, y_train, x_test, y_test = cai.datasets.load_dataset(
        dataset,
        verbose=verbose,
        lab=lab,
        bipolar=bipolar,
...
def train_model_on_cifar10(
    ...
    # Returns
        ...
        csv_name: string with CSV file name showing training progress.
    """
    return train_model_on_dataset(
        model=model,
        dataset=cifar10,
        base_model_name=base_model_name,
        plrscheduler=plrscheduler,
        batch_size=batch_size,
        epochs=epochs,
        momentum=momentum,
        nesterov=nesterov,
...
def load_image_file_names_from_folder(img_folder_name):
    ...
    for img_file in img_list:
        ...
        if (
            absolute_file_name_lower.endswith(".jpg")
            or absolute_file_name_lower.endswith(".jepg")
            or absolute_file_name_lower.endswith(".png")
        ):
            output_img_list.append(absolute_file_name)
    return output_img_list


def flip_images_on_folder(
    img_folder_name, with_horizontal_flip=True, with_vertical_flip=False
):
    ...
    for img_file in img_list:
        ...
        if (
            ...
            if with_horizontal_flip:
                cv2.imwrite(h_absolute_dest_file_name, np.flip(img, 1))
            if with_vertical_flip:
                cv2.imwrite(v_absolute_dest_file_name, np.flip(img, 0))
            if with_horizontal_flip and with_vertical_flip:
                cv2.imwrite(hv_absolute_dest_file_name, np.flip(np.flip(img, 0), 1))
    ...
    for file_name in file_names:
        ...
        if smart_resize:
            ...
            if (img.shape[0] != target_size[0]) or (img.shape[1] != target_size[1]):
                img = cv2.resize(
                    img, dsize=target_size, interpolation=cv2.INTER_NEAREST
                )
        else:
...
def load_images_from_folders(
    ...
    for i, _class in enumerate(classes):
        ...
        paths = [
            ...
            or n.lower().endswith(".jpeg")
            or n.lower().endswith(".tif")
            or n.lower().endswith(".tiff")
            or n.lower().endswith(".bmp")
        ]
        ...
        if validation_size > 0:
            val_path.extend(
                paths[
                    int(cat_total * training_size) : int(
                        cat_total * (training_size + validation_size)
                    )
                ]
            )
            val_y.extend(
        ...
        if test_size > 0:
            if training_size + validation_size + test_size >= 1:
                test_path.extend(
                    paths[int(cat_total * (training_size + validation_size)) :]
                )
                test_y.extend(
                    [i]
                    * len(paths[int(cat_total * (training_size + validation_size)) :])
                )
            else:
                test_path.extend(
                    paths[
                        int(cat_total * (training_size + validation_size)) : int(
    ...
    if has_training:
        ...
        train_x = np.array(
            cai.datasets.load_images_from_files(
                train_path,
...
def test_flips_on_model(
    ...
    if center_crop > 0:
        ...
        cropped_test_x = test_x[
            :,
            center_crop_py : size_y - center_crop_py,
            center_crop_px : size_x - center_crop_px,
            :,
        ]
...
def test_flips_on_saved_model(
    ...
    test_flips_on_model(
        ...
        test_y=test_y,
        model=model,
        has_flip_x=has_flip_x,
        has_flip_y=has_flip_y,
        has_bw=has_bw,
...
def extract_subset_every(
    source_folder, dest_folder, move_every=10, shift=0, verbose=False
):
    """
    ...
    for subfolder in subfolders:
        ...
        file_list = sorted(
            [
                ...
                if os.path.isfile(class_folder + "/" + filename)
            ]
        )
