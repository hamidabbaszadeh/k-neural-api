    ...
    for depthCnt in range(Depth):
        ...
        if ForceCellMax:
            ...
            if SliceMax > 0:
                Slice /= SliceMax
            aResult[
                PosX * SizeX : (PosX + 1) * SizeX, PosY * SizeY : (PosY + 1) * SizeY
            ] += Slice
        else:
            aResult[
                PosX * SizeX : (PosX + 1) * SizeX, PosY * SizeY : (PosY + 1) * SizeY
    ...
    for NeuronsCnt in range(Neurons):
        ...
        if ForceCellMax:
            ...
            if SliceMax > 0:
                Slice /= SliceMax
            aResult[
                PosX + PosX * SizeX : PosX + (PosX + 1) * SizeX,
                PosY + PosY * SizeY : PosY + (PosY + 1) * SizeY,
                :,
            ] += Slice
        else:
            aResult[
...
def preprocess(img, bipolar=True, tfcons=False):
    ...
    if bipolar:
        ...
        img /= 255
    if tfcons:
        img = tensorflow.constant(np.array(img))
...
def deprocess(img, bipolar=True, tfcast=False):
    ...
    if bipolar:
        ...
        img *= 255
    if tfcast:
        img = tensorflow.cast(img, tensorflow.uint8)
